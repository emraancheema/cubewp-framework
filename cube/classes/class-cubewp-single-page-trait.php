...
trait CubeWp_Single_Page_Trait {
	 ...
	 * @since  1.0.0
	 */
	private static function field_taxonomy($args = array()) {
		...
		if (isset($args['value']) && ! empty($args['value']) && is_array($args['value'])) {
			...
			$output .= '<ul class="cwp-single-category-widget-inner">';
	...
	* @param array $args field data
	 ...
	 * @since  1.0.0
	 */
	private static function field_gallery($args = array()) {
		$args['field_size'] = isset($args['field_size']) ? $args['field_size'] : '';
		$output = null;
		if (is_array($args['value'])) {
			$output .= '<div class="cwp-cpt-single-gallery-container cwp-cpt-single-field-container ' . esc_attr($args['container_class']) . ' '.esc_attr($args['field_size']).'">
                <h4>' . $args['label'] . '</h4>
                <div class="cwp-cpt-single-gallery">';
	 ...
	 * @since  1.0.0
	 */
	private static function field_google_address($args = array()) {
		$args['field_size'] = isset($args['field_size']) ? $args['field_size'] : '';
		$output = null;
	...
	* @param array $args field data
	 ...
	 * @since  1.0.0
	 */
	private static function field_repeating_field($args = array()) {
		$args['field_size'] = isset($args['field_size']) ? $args['field_size'] : '';
        $values = $args['value'];
		$output = '';
		...
		$output .= '<div class="cwp-cpt-single-repeating-field cwp-cpt-single-field-container '.esc_attr($args['container_class']).' '.esc_attr($args['field_size']).'">
            <h4>' . $args['label'] . '</h4>';
				for ($i = 0; $i < count($values); $i ++) {
					$output .= '<div class="cwp-cpt-single-repeating-field-inner hggfhgfh">';
	 ...
	 * @since  1.0.0
	 */
	private static function field_terms($args = array()) {
		$args['field_size'] = isset($args['field_size']) ? $args['field_size'] : '';
		$output = null;
		if (isset($args['value']) && ! empty($args['value']) && is_array($args['value']) ) {
			$output = '<div class="cwp-cpt-single-category-container cwp-cpt-single-field-container '.esc_attr($args['container_class']).' '.esc_attr($args['field_size']).'">';
	 ...
	 * @since  1.0.0
	 */
	private static function field_user($args = array()) {
		$args['field_size'] = isset($args['field_size']) ? $args['field_size'] : '';
		$output = null;
        //cwp_pre($args);
		if (isset($args['value']) && ! empty($args['value'])) {
			$output .= '<div class="cwp-cpt-single-user-container cwp-cpt-single-field-container '.esc_attr($args['container_class']).' '.esc_attr($args['field_size']).'">
				<h4>' . $args['label'] . '</h4>';
            $output .= '<div class="cwp-row cwp-user-row">';
			if (is_array($args['value'])) {
				...
				$user_data = get_userdata($args['value']);
				if (!empty($user_data) && is_object($user_data)) {
					$output .= '<div class="cwp-col-md-6">';
                    $output .= get_user_details($args['value']);
	                $output .= '</div>';
				}
			}
			$output .= '</div>
	...
	* @param array $args field data
	 ...
	 * @since  1.0.0
	 */
	private static function field_post($args = array()) {
		wp_enqueue_style('loop-style');
		$args['field_size'] = isset($args['field_size']) ? $args['field_size'] : '';
		$output = null;
		if (isset($args['value']) && !empty($args['value']) && $args['value'] != 'N/A') {
			$output .= '<div class="cwp-cpt-single-post-container cwp-cpt-single-field-container '.esc_attr($args['container_class']).' '.esc_attr($args['field_size']).'">
				<h4>' . $args['label'] . '</h4>';
            $output .= '<div class="cwp-row cwp-post-row">';
			if (is_array($args['value'])) {
				foreach ($args['value'] as $post_id) {
					$output .= CubeWp_frontend_grid_HTML($post_id);
				}
			}else{
				$output .= CubeWp_frontend_grid_HTML($args['value']);
			}
			$output .= '</div>
            </div>';
		}
	 ...
	 * @since  1.0.0
	 */
	private static function field_image($args = array()) {
		$args['field_size'] = isset($args['field_size']) ? $args['field_size'] : '';
		$output = null;
		...
		if (isset($args['value']) && !empty ($imageURL)) {
			$output .= '<div class="cwp-cpt-single-image-container cwp-cpt-single-field-container ' . esc_attr($args['container_class']) . ' '.esc_attr($args['field_size']).'">
                <h4>' . $args['label'] . '</h4>
                <div class="cwp-cpt-single-image">';
				$imageURL     = wp_get_attachment_url($args['value']);
    ...
    public static function get_post_share_button($post_id = '') {
        ...
        echo '
        <div class="cwp-single-share-btn cwp-single-widget">
            <span class="cwp-main">
                 <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-share-fill" viewBox="0 0 16 16">
                  <path d="M11 2.5a2.5 2.5 0 1 1 .603 1.628l-6.718 3.12a2.499 2.499 0 0 1 0 1.504l6.718 3.12a2.5 2.5 0 1 1-.488.876l-6.718-3.12a2.5 2.5 0 1 1 0-3.256l6.718-3.12A2.5 2.5 0 0 1 11 2.5z"/>
                </svg>
                <span class="cwp-share-text">Share</span>
            </span>
            <div class="cwp-share-modal" style="display:none;">
                <ul class="cwp-share-options">';
                    if($cwpOptions['twitter_share']=='1'){
                        echo '<li style="background-color: #4099FF;">
                            '. CubeWp_Single_Page_Trait::get_twitter_svg(esc_url($twitterURL)) .'
                        </li>';
                    }
                    if($cwpOptions['facebook_share']=='1'){
                        echo '<li style="background-color: #3b5998;">
                            '. CubeWp_Single_Page_Trait::get_facebook_svg(esc_url($facebookURL)) .'
                        </li>';
                    }
                    if($cwpOptions['pinterest_share']=='1'){
                        echo '<li style="background-color: #C92228;">
                            '. CubeWp_Single_Page_Trait::get_pinterest_svg(esc_url($pinterest)) .'
                        </li>';
                    }
                    if($cwpOptions['linkedin_share']=='1'){
                        echo '<li style="background-color: #0077B5;">
                            '. CubeWp_Single_Page_Trait::get_linkedIn_svg(esc_url($linkedin)) .'
                        </li>';
                    }
                    if($cwpOptions['reddit_share']=='1'){
                        echo '<li style="background-color: #fe6239;">
                            '. CubeWp_Single_Page_Trait::get_reddit_svg(esc_url($reddit)) .'
                        </li>';
                    }
            echo '</ul>
            </div>
